...
export function CoastlineAndCommunity({ language }: { language: ReadinessLanguage }) {
  const fr = language === "fr";
  return <section className="coast-community"><article><MapPinned size={20} /><p className="mono-label">{fr ? "RÉFÉRENCE DE LITTORAL" : "COASTLINE REFERENCE"}</p><h2>{fr ? "Un littoral peut être cartographié sans devenir une preuve d’impact." : "A coastline can be mapped without becoming impact proof."}</h2><p>{fr ? "Annual Shorelines (Landsat, 30 m) est une série vectorielle régionale issue d’images Landsat, à partir de 1980. Cette salle fournit la méthode et le lien, mais n’intègre pas de géométrie qui nécessite une demande d’accès et ne confond pas rivage, ZEE et anomalie de niveau marin." : "Annual Shorelines (Landsat, 30 m) is a regional vector time series derived from Landsat imagery, from 1980. This room provides method and link, but does not embed geometry that requires access approval or conflate coast, EEZ, and sea-level anomaly."}</p><a href={coastlineUrl} target="_blank" rel="noreferrer">{fr ? "Lire la fiche et la licence" : "Read dataset record and licence"} <ExternalLink size={14} /></a></article><article className="community-slot"><Pin size={20} /><p className="mono-label">{fr ? "UNE VOIX PUBLIQUE, CITÉE AVEC ATTRIBUTION" : "A PUBLIC VOICE, QUOTED WITH ATTRIBUTION"}</p><h2>{fr ? "Les données ne remplacent pas la parole." : "Data does not replace a voice."}</h2><blockquote className="community-slot__quote">{fr ? <>« the sand is slipping between your fingers »</> : <>&ldquo;the sand is slipping between your fingers&rdquo;</>}<cite>{fr ? "— Cynthia Houniuhi, présidente, Pacific Islands Students Fighting Climate Change (Îles Salomon)" : "— Cynthia Houniuhi, President, Pacific Islands Students Fighting Climate Change (Solomon Islands)"}</cite></blockquote><p>{fr ? "Elle décrivait, dans une interview publique, ce que le changement climatique fait ressentir. Ce n’est pas un témoignage sollicité pour ce projet\u00a0: c’est une déclaration publique, courte, citée avec attribution exacte et un lien vers la source d’origine. Les Îles Salomon figurent parmi les lieux relevés dans cette salle." : "She was describing, in a public interview, what climate change feels like. This is not testimony solicited for this project — it is a short public statement, quoted with precise attribution and a link to the original source. The Solomon Islands is one of the places recorded elsewhere in this room."}</p><a href="https://www.pbs.org/newshour/show/young-pacific-islanders-take-industrialized-world-to-court-over-climate-change" target="_blank" rel="noreferrer">{fr ? "Voir l’interview originale, PBS NewsHour, 13 avril 2023" : "Watch the original interview, PBS NewsHour, April 13, 2023"} <ExternalLink size={13} /></a></article></section>;
}
